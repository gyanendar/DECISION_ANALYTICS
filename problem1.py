...
class logic_puzzel:
    ...
    def create_decision_variable(self):
        ...
        for person in self.persons:        
            ...
            for starter in self.starters:    
                variables[starter] = self.model.NewBoolVar(person+"<->"+starter)
        ...
        for person in self.persons:        
            ...
            for main_course in self.main_courses:    
                variables[main_course] = self.model.NewBoolVar(person+"<->"+main_course)
        ...
        for person in self.persons:        
            ...
            for desert in self.deserts:    
                variables[desert] = self.model.NewBoolVar(person+"<->"+desert)
        ...
        for person in self.persons:        
            ...
            for drink in self.drinks:    
                variables[drink] = self.model.NewBoolVar(person+"<->"+drink)
    ...
    def implicit_constraint(self):
        ...
        for p_index in range(len(self.persons)):
            for p_index_next in range(p_index+1,len(self.persons)):
                ...
                for s_index in range(len(self.starters)):
                    self.model.AddBoolOr([self.person_starter[self.persons[p_index]][self.starters[s_index]].Not(),
                                    self.person_starter[self.persons[p_index_next]][self.starters[s_index]].Not()])
                #Every person taking different main courses    
                for m_index in range(len(self.main_courses)):
                    self.model.AddBoolOr([self.person_main_course[self.persons[p_index]][self.main_courses[m_index]].Not(), 
                                    self.person_main_course[self.persons[p_index_next]][self.main_courses[m_index]].Not()])
                #Every person taking different desert
                for d_index in range(len(self.deserts)):
                    self.model.AddBoolOr([self.person_desert[self.persons[p_index]][self.deserts[d_index]].Not(), 
                                    self.person_desert[self.persons[p_index_next]][self.deserts[d_index]].Not()])
                #Every person taking different drink
                for dk_index in range(len(self.drinks)):
                    self.model.AddBoolOr([self.person_drink[self.persons[p_index]][self.drinks[dk_index]].Not(), 
                                    self.person_drink[self.persons[p_index_next]][self.drinks[dk_index]].Not()])
        ...
        for person in self.persons:
            ...
            variables = []
            for starter in self.starters:
                variables.append(self.person_starter[person][starter])
            self.model.AddBoolOr(variables)

            variables = []
            for main_course in self.main_courses:
                variables.append(self.person_main_course[person][main_course])
            self.model.AddBoolOr(variables)

            variables = []
            for desert in self.deserts:
                variables.append(self.person_desert[person][desert])
            self.model.AddBoolOr(variables)

            variables = []
            for drink in self.drinks:
                variables.append(self.person_drink[person][drink])
            self.model.AddBoolOr(variables)                                    
            ...
            for i in range(len(self.starters)):
                for j in range(i+1,len(self.starters)):
                    self.model.AddBoolOr([
                            self.person_starter[person][self.starters[i]].Not(), 
                            self.person_starter[person][self.starters[j]].Not()])

            for i in range(len(self.person_main_course)):
                for j in range(i+1,len(self.person_main_course)):                
                    self.model.AddBoolOr([
                            self.person_main_course[person][self.main_courses[i]].Not(), 
                            self.person_main_course[person][self.main_courses[j]].Not()])

            for i in range(len(self.deserts)):
                for j in range(i+1,len(self.deserts)): 
                    self.model.AddBoolOr([
                            self.person_desert[person][self.deserts[i]].Not(), 
                            self.person_desert[person][self.deserts[j]].Not()])

            for i in range(len(self.drinks)):
                for j in range(i+1,len(self.drinks)):
                    self.model.AddBoolOr([
                            self.person_drink[person][self.drinks[i]].Not(), 
                            self.person_drink[person][self.drinks[j]].Not()]) 
    ...
    def ex_constraint_1(self):
        ...
        self.model.AddBoolAnd([self.person_starter["Emily"]["prawn_cocktail"].Not()])
        self.model.AddBoolAnd([self.person_main_course["Emily"]["baked_mackerel"].Not()])

    def ex_constraint_2(self):
        # Explicit constraint
        # Daniel does not want the prawn cocktail as starter and James does not drink beer        
        self.model.AddBoolAnd([self.person_starter["Daniel"]["prawn_cocktail"].Not()])
        self.model.AddBoolAnd([self.person_drink["James"]["beer"].Not()])

    def ex_constraint_3(self):
        # Explicit constraint
        # Sophie will only have fried chicken as main course if she does not have to take the prawn cocktail as starter
        
        # So Sophie will have either fried chicken as main course or prawn cocktail as starter but not both together
                        
        self.model.AddBoolOr([self.person_main_course["Sophie"]["fried_chicken"].Not(),
                              self.person_starter["Sophie"]["prawn_cocktail"].Not()])
    ...
    def ex_constraint_4(self):
        # Explicit constraint
        # The filet steak main course should be combined with the onion soup as starter and with the apple crumble for dessert
        
        for person in self.persons:        
            self.model.AddBoolAnd([self.person_starter[person]["onion_soup"],
                                   self.person_desert[person]["apple_crumble"]]).OnlyEnforceIf(self.person_main_course[person]["filet_steak"])
        
    def ex_constraint_5(self):
        #Explicit constraint
        #The person who orders the mushroom tart as starter also orders the red wine
        for person in self.persons:        
            self.model.AddBoolAnd([self.person_starter[person]["mushroom_tart"]]).OnlyEnforceIf(self.person_drink[person]["red_wine"])
    ...
    def ex_constraint_6(self):
        ...
        for person in self.persons:
            self.model.AddBoolOr([self.person_main_course[person]["baked_mackerel"].Not(),self.person_desert[person]["ice_cream"].Not()])   
                 
            self.model.AddBoolOr([self.person_main_course[person]["vegan_pie"].Not(),self.person_starter[person]["prawn_cocktail"].Not()])        
            self.model.AddBoolOr([self.person_main_course[person]["vegan_pie"].Not(),self.person_starter[person]["carpaccio"].Not()])        
    ...
    def ex_constraint_8(self):
        ...
        self.model.AddBoolOr([self.person_drink["Emily"]["white_wine"],self.person_drink["Emily"]["red_wine"]])
        self.model.AddBoolOr([self.person_drink["Sophie"]["white_wine"],self.person_drink["Sophie"]["red_wine"]])
    
    def ex_constraint_9(self):
        #Explicit constraint
        #One of the men has chocolate cake for dessert, while the other prefers not to have ice cream 
        #or coke but will accept one of the two if necessary 
        #
        
        #
        #One of the men has chocolate cake for dessert 
        self.model.AddBoolXOr([self.person_desert["James"]["chocolate_cake"],self.person_desert["Daniel"]["chocolate_cake"]])        
        ...
        self.model.AddBoolOr([self.person_desert["James"]["ice_cream"].Not(),self.person_drink["James"]["coke"].Not()]).OnlyEnforceIf(self.person_desert["Daniel"]["chocolate_cake"])
        #if James takes chocolate_cake than Daniel wont take ice-cream of coke
        self.model.AddBoolOr([self.person_desert["Daniel"]["ice_cream"].Not(),self.person_drink["Daniel"]["coke"].Not()]).OnlyEnforceIf(self.person_desert["James"]["chocolate_cake"])

        # But they accept one if necessary
        #
        # if James takes chocolate_cake as desert, James will not have ice cream anyway. so he can take coke
        self.model.AddBoolAnd([self.person_drink["James"]["coke"].Not()]).OnlyEnforceIf(self.person_desert["James"]["chocolate_cake"])
        # if Daniel takes chocolate_cake as desert, James will not have ice cream anyway. so he can take coke
        self.model.AddBoolAnd([self.person_drink["Daniel"]["coke"].Not()]).OnlyEnforceIf(self.person_desert["Daniel"]["chocolate_cake"])
...
class soduku():
        ...
        for row in range(9):
            ...
            for col in range(9):
                ...
                for var in range(1,10):
                    ...
                    variables[var] = self.model.NewBoolVar(f"Var_[{row}][{col}]_[{var}]")
    ...
    def create_constraints(self):
            ...
            for col in range(9):
                for next_col in range(col+1,9):
                    for var_cell in range(1,10):
                        self.model.AddBoolOr([self.soduku_variables[row][col][var_cell].Not(),self.soduku_variables[row][next_col][var_cell].Not()])
        ...
        for col in range(9):
            for row in range(9):
                for next_row in range(row+1,9):
                    for var_cell in range(1,10):
                        self.model.AddBoolOr([self.soduku_variables[row][col][var_cell].Not(),self.soduku_variables[next_row][col][var_cell].Not()])
        ...
        for item in square_indexes:
            ...
            for seq in range(len(indexs)):
                for next_seq in range(seq+1,len(indexs)):
                    for var_cell in range(1,10):
                        self.model.AddBoolOr([self.soduku_variables[indexs[seq][0]][indexs[seq][1]][var_cell].Not(),
        ...
        for item in square_indexes:
            ...
            for seq in range(len(indexs)):
                variables=[]
                for var_cell in range(1,10):
                    variables.append(self.soduku_variables[indexs[seq][0]][indexs[seq][1]][var_cell])
                self.model.AddBoolOr(variables)    
    ...
    def initialise_soduku_cells(self):

        for row in range(9):
            for col in range(9):
                if self.initial_value[row][col]!=0:
                    self.model.AddBoolAnd([self.soduku_variables[row][col][self.initial_value[row][col]]])
...
class project_planning():
    ...
    def load_excel_and_read(self):
        ...
        project_df = pd.read_excel(self.xls_file_path,sheet_name="Projects")
        quotes_df = pd.read_excel(self.xls_file_path,sheet_name="Quotes")
        dependency_df = pd.read_excel(self.xls_file_path,sheet_name="Dependencies")
        value_df = pd.read_excel(self.xls_file_path,sheet_name="Value")

        # first columns doesnt have any header name
        # put header name as per data
        project_df.rename(columns={'Unnamed: 0':'Project'}, inplace=True)
        quotes_df.rename(columns={'Unnamed: 0':'contractor'}, inplace=True)
        dependency_df.rename(columns={'Unnamed: 0':'Project'}, inplace=True)
        value_df.rename(columns={'Unnamed: 0':'Project'}, inplace=True)

        return project_df,quotes_df,dependency_df,value_df

    def get_names(self):
        self.project_names = list(self.project_df["Project"])
        self.contractors_names = list(self.quotes_df["contractor"])
        self.jobs_name = list(self.quotes_df.columns)[1:]
        self.month_name = list(self.project_df.columns)[1:]

    def task_A(self):
        self.project_df,self.quotes_df,self.dependency_df,self.value_df = self.load_excel_and_read()
        self.get_names()
    ...
    def create_variables(self):
        ...
        # project = [A,B,C..]
        # B>
        # This will answer "Identify and create the decision variables in a CP-SAT model that you need to decide 
        # what projects to take on  "
        # Create variable names for each project
        
        for project in self.project_names:
            self.project_variables[project] = self.model.NewBoolVar(f"var_{project}")

        # VARIABLE CREATION
        # B> which contractor is working on which project and when
        # Project consists of job which needs to be completed in particular month by a contractor
            ...
            for month in self.month_name:
                ...
                for job in self.jobs_name:
                    ...
                    for contractor in self.contractors_names:
                        job_var[contractor] = self.model.NewBoolVar(f"var_{project}_{month}_{job}_{contractor}")
    ...
    def create_mapping_lookup(self):
        ...
        for item in self.value_df.to_dict("records"):        
        ...
        for project in self.project_names:        
            for _,job in self.project_job_month[project]:           
                for month in self.month_name:
                    variables=[]
                    for contractor in self.contractors_names:
                        variables.append(self.project_month_job_contractor[project][month][job][contractor])
                    self.model.Add(sum(variables) <= 1).OnlyEnforceIf(self.project_variables[project])
    ...
    def constraint_B(self):
        ...
        #
        # if contractor is not qualified for a job,there wont be any quote for that job by the contractor
        #
        ...
        for contractor in self.contractors_names:
            for job in self.jobs_name:            
                if (0 == self.contractor_job_quote[contractor].get(job,0)):
                    ...
                    for project in self.project_names:
                        for month in self.month_name:
                            variables.append(self.project_month_job_contractor[project][month][job][contractor].Not())
    ...
    def constraint_D(self):
        ...
        # D>project is accepted to be delivered 
        ...
        for project in self.project_names:
            ...
            for month in self.month_name:
                for job in self.jobs_name:
                    for contractor in self.contractors_names:
                        variables.append(self.project_month_job_contractor[project][month][job][contractor])
            self.model.Add(sum(variables)==0).OnlyEnforceIf(self.project_variables[project].Not())     
        ...
        for _, row in self.dependency_df.iterrows():          
            for project in self.project_names:
                if(row[project] == 'required'):                
                    self.model.AddBoolOr([self.project_variables[row["Project"]].Not(),self.project_variables[project]])
                elif (row[project] == 'conflict'):
                    self.model.AddBoolOr([self.project_variables[row["Project"]].Not(),self.project_variables[project].Not()]) 
    ...
    def constraint_G(self):
        ...
        earning_per_project =[]          
        for project in self.project_names:
            earning_per_project.append(self.project_value[project] * self.project_variables[project])
            ...
            for month in self.month_name:
                for job in self.jobs_name:
                    for contractor in self.contractors_names:
                        ...
                        expense_per_project.append(cost*self.project_month_job_contractor[project][month][job][contractor])
        ...
        for project in self.project_names:
            for month in self.month_name:            
                for contractor in self.contractors_names:
                    for job in self.jobs_name:
                        self.model.AddBoolOr([
                                        self.project_month_job_contractor[project][month][job][contractor].Not(),
...
def problem_1():
    ...
    status = solver.SearchForAllSolutions(model, logic_puzzel_SolutionPrinter(puzzle_obj))
...
def problem_2():
    ...
    status = solver.SearchForAllSolutions(model, soduku_SolutionPrinter(soduku_obj))
    print(solver.StatusName(status))   
...
def problem_3():
    ...
    status = solver.SearchForAllSolutions(model,project_planner_SolutionPrinter(project_planner_obj))
    print(solver.StatusName(status))
